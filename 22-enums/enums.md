...
Enumerated types (enums) are a special case of sum types. An enum is a type
that has a fixed number of possible values, each with a distinct name. Go
doesn't have an enum type as a distinct language feature, but enums are simple
to implement using existing language idioms.
...
keyword `iota` generates successive constant values automatically; in this
...
`stringer` tool can be used in conjunction with `go:generate` to automate the
process.
